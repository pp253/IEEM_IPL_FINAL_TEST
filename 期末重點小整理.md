...
這邊只說比較難懂的，其他東西已經在 [函式庫小抄](https://gist.github.com/pp253/1fd3a19b19d241ceb9d3e9da91ceea2f) 裡面大概講過了。
...
來說說 `struct` 要怎麼拿他的值呢？我們教的都是 `someStruct->abc`，但事實上還有 `someStruct.abc` 歐。
...
主持人在台上放了與參加者人數同顆不同顏色的球，這樣才人人有獎嘛～
...
現在球已經被選完了，主持人就請剛剛這些參賽者上台把她們拿到的球按照選球的順序秀給觀眾看。
...
第幾位參加者： `member` (注意：第一位參加者將會是0)

「哪些球被拿走了」列表： `taken`

「後台休息區」列表： `backstage`

現在我們的任務是要用以上面數模擬以上的電視節目。好的，現在第一位參加者(注意：她的編號是0號)要上場選球了。
...
void choose (int total, int member, int *taken) {
  int i;
  for (i = 0; i < total; i++) {
    if (taken[i] == 0) { // 這句話代表：如果第i顆球還在的話，那參賽者就可以拿
      taken[i] = 1; // 第i顆球被這位參賽者拿走了～
      choose (total, member + 1, taken); // 換下一位參賽者
      taken[i] = 0; // 第i顆球被這位參賽者放回來了～嗯？奇怪？
    }
  }
}
```

等等，為什麼要把球放回去？別忘了！我們要模擬「所有可能」，如果那位參加者不放回來，那那顆球就等於永遠消失不見了！所以他在模擬完一種可能之後一定要放回來。
...
void choose (int total, int member, int *taken) {
  int i;
  for (i = 0; i < total; i++) {
    if (taken[i] == 0) { // 這句話代表：如果球還在的話，那參賽者就可以拿
      taken[i] = 1; // 球被這位參賽者拿走了～

      backstage[member] = i; // 坐在後台的第member位參加者手中拿著第i顆球
                             // 因為他拿走了嘛~
      
      choose (member + 1); // 換下一位參賽者
      taken[i] = 0; // 球被這位參賽者放回來了
    }
  }
}
```
